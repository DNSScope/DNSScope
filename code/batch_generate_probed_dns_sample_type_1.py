...
my_output_path = r'../data/probe_sample_type_1/'
...
def get_probe_sample(cri_file_name, rate_file_name):
    period = 86400
    window_size = 600
    if exists(cri_file_name) is False or exists(rate_file_name) is False:
        return None
    ...
    probe_sample = [[[], rate_list[i]] for i in range(int(np.ceil(period / window_size)))]
    ...
    return probe_sample, max_rate, rate_tag


def generate_trace(task_info):
    domain_name, ttl, cri_file_name, rate_file_name, dns_server = task_info
    trace_tag = domain_name + '_' + dns_server
    probe_sample, max_rate, rate_tag = get_probe_sample(cri_file_name, rate_file_name)
    try:
        features = get_features(probe_sample)
        rates = [x[1] for x in probe_sample]
        # time_stamp = str(datetime.datetime.now().timestamp())
        sample_file_name = my_output_path + 'sample_' + str(max_rate) + '_' + str(ttl) + '_' + str(trace_tag) +'_'+str(0)+ '.pkl'
    ...
    for ttl in ttl_table.keys():
        for dir_path, folders, files in walk(ttl_table[ttl][0]):
            for folder in folders:
                ...
                ttl_table[ttl][1].append([domain_name, cri_file_name, rate_file_name, dns_server])
            break
    for ttl in ttl_table.keys():
...
if __name__ == '__main__':
    ...
    my_sample_path = r'../data/hybrid_r_dns_80/CRIs_type1/'
