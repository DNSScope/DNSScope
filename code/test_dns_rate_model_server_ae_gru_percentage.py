...
import pandas as pd
import pickle
from batch_generate_dns_sample import get_features_in_estimation, generate_trace_direct, get_mix_exp_rate
...
model_path = r'../model/'
testing_sample_path = r'../data/real_trace/'
...
def generate_rate_samples(my_mapper, estimated_rates):
    ...
    for i in range(repeat_num):
        ...
        # error_ratios = my_mapper.shift(estimated_data, 0.005)
        # error_ratios = my_mapper.shift(copy.deepcopy(estimated_data), 0)
        error_ratios = np.minimum(np.maximum(error_ratios, min_error_ratio), max_error_ratio)
...
def rate_estimation_int_interval(task_info):
    # print('#'*100)
    testing_samples, my_predictor, model_tag, loss_weight, TTL, trace_tag, sample_percentage = task_info
    ...
    # my_predictor, testing_samples, TTL, trace_tag = task_info
    predictor_raw = copy.deepcopy(my_predictor).to(device)
    testing_rates, estimation_list, ML_estimation_list, UB_estimation_list, estimation_info_1 = rate_estimate(predictor_raw, copy.deepcopy(testing_samples), trace_tag)
    result_list = []
    ae_model_file_name = model_path + 'AE_GRU_' + str(TTL) + '_' + str(model_tag) + '.model'
    my_mapper = torch.load(ae_model_file_name, map_location=device)
    ...
    if is_retrain:
        ...
        for _ in range(retrain_num):
            ...
            for i in range(estimation_list.shape[1]):
                ...
                rate_samples.extend(generate_rate_samples_guided(my_mapper, estimation_list[:, i], mix_exp_rates[i]))
            ...
            testing_rates, retrain_estimation_list, _, _, estimation_info_2 = rate_estimate(predictor, copy.deepcopy(
                testing_samples), trace_tag)
...
if __name__ == '__main__':
    ...
    result_path = r'../result/campus_1500_result/'
    ...
    my_domain_list = np.array(pd.read_csv(r'my_domain_list.txt', header=None)).flatten()
