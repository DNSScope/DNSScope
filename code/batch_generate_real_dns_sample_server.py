...
from simulate_dns_arrival_sample import simulate_probe_from_file
import datetime
...
def generate_trace(task_info):
    trace_file_name, TTL = task_info
    result_list = []
    probe_sample_list = simulate_probe_from_file(trace_file_name, TTL=TTL, second_precision=False)
    if probe_sample_list is None:
        return result_list
    for probe_sample, max_rate, TTL, trace_tag in probe_sample_list:
        features = get_features(probe_sample)
        rates = [x[1] for x in probe_sample]
        result_list.append([features, rates, probe_sample, max_rate, TTL, trace_tag])
    return result_list
    ...
    for dir_path, folders, files in walk(sample_path):
        for file_name in files:
            ...
            if existing_domain_dns.__contains__((domain_name, dns_server)):
                continue
            file_list.append([sample_path + file_name, domain_table[domain_name]])
    return file_list
...
def run():
    ...
    for result_list in pool.imap_unordered(generate_trace, task_list):
        for i in range(len(result_list)):
            ...
            features, rates, probe_sample, max_rate, TTL, trace_tag = result
            sample_file_name = output_path + 'sample_' + str(max_rate) + '_' + str(TTL) + '_' + str(trace_tag) +'_'+str(i)+ '.pkl'
