...
# my_percentage_list = [0.5, 0.2, 0.1]
...
model_path = r'../model/'
testing_sample_path = r'../data/real_trace_multiple/'
...
def rate_estimation_int_interval(task_info):
    ...
    for results in all_result_list:
        merged_results[:results.shape[0], :results.shape[1]] = merged_results[:results.shape[0], :results.shape[1]] + np.maximum(results, 0)
...
def evaluate_model(testing_sample_table):
    ...
    for trace_tag in testing_sample_table.keys():
        ...
        for my_model, model_tag, loss_weight, sample_percentage in model_list:
            # for sample in testing_sample_table[trace_tag]:
            #     task_info = [sample, my_model, model_tag, loss_weight, active_ttl, trace_tag, sample_percentage]
...
def list_testing_file(sample_path):
    ...
    for trace_tag in file_name_table.keys():
        file_name_table[trace_tag] = list(sorted(file_name_table[trace_tag]))
...
if __name__ == '__main__':
    ...
    if len(sys.argv) > 2:
        my_cache_num = int(sys.argv[2])
    if len(sys.argv) > 3:
        my_drift_factor = float(sys.argv[3])
    is_force_estimate = False
    # is_force_estimate = True

    result_path = r'../result/campus_1500_result_multiple_cache_' + str(my_cache_num) + '/'

    testing_sample_path += str(my_cache_num)+'/'
