...
def get_mix_exp_rate(probe_sample,TTL):
    ...
    for _ in range(repeat_num):
        # intervals = [get_float_interval(int(np.maximum(np.round(x)-1, 0)), estimated_rate) for x in intervals if x != int(x)]
        for j in range(len(intervals)):
            if int(intervals[j]) != intervals[j]:
                intervals[j] = get_float_interval(int(np.maximum(np.round(intervals[j]) - offset, 0)), estimated_rate)
                # intervals[j] = get_float_interval(0, estimated_rate)
            else:
                intervals[j] = get_float_interval(int(np.maximum(np.round(intervals[j]), 0)), estimated_rate)
...
def get_features_in_estimation(probe_sample_1, estimated_rates):
    probe_sample = copy.deepcopy(probe_sample_1)
    for i in range(len(probe_sample)):
        intervals = probe_sample[i][0]
        # intervals = [int(x) for x in intervals]
        # intervals = [int(np.maximum(np.round(x)-offset, 0)) for x in intervals if x >= 0]
        intervals = [int(np.maximum(np.round(x) - offset, 0)) for x in intervals]
    ...
    # for i in range(len(probe_sample)): # reset intervals
    #     intervals = probe_sample[i][0]
    #     intervals = [int(x) for x in intervals]
    #     probe_sample[i][0] = intervals
...
def run(task_infos):
    ...
    for task_info in task_infos:
        ...
        arrival_num = task_info[3]
        rate_num = task_info[4]
        m_num = task_info[5]
